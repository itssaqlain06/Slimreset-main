                                            ...
                                            <div id="basic-1_wrapper" class="dataTables_wrapper no-footer">
                                                <table class="display dataTable no-footer" id="basic-1" role="grid" aria-describedby="basic-1_info">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        ?>
                                                            <tr role="row" class="odd" id="veggie_<?php echo $row['id']; ?>">
                                                                <td>
                                                                ...
                                                                <td>
                                                                    <ul class="action" style="list-style-type: none; padding: 0; margin: 0; display: flex; gap: 10px;">
                                                                        <li class="edit">
                                                                            <a href="edit-veggie.php?id=<?php echo $row['id']; ?>" title="Edit">
            ...
            $('.status-link').on('click', function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: 'POST',
                            url: '../functions/recipes/veggie/status.php',
                            data: {
                                id: veggie,
                            ...
                            success: function(response) {
                                if (response.trim() === 'Success') {
                                    $('a[data-id="' + veggie + '"]').text(newStatus).toggleClass('active inactive');
                                    Swal.fire({
                                        title: 'Success',
        ...
        $(document).ready(function() {
            $('.delete-record').on('click', function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '../functions/recipes/veggie/delete.php',
                            type: 'POST',
                            data: {
                                id: recordId
                            },
                            success: function(response) {
                                if (response.trim() === 'Success') {
                                    Swal.fire(
                                        'Deleted!',
                                        'The veggie item has been deleted.',
                                        'success'
                                    ).then(() => {
                                        location.reload();
                                    });
